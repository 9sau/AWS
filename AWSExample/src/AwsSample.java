	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			throw new AmazonClientException("Cannot load the credentials from the credential profiles file. "
					...
					+ "location (/Users/Saurabh/.aws/credentials), and is in valid format.", e);
		}
		...
		try {
			...
			// Open up port 23 for TCP traffic to the associated IP from above
			// (e.g. ssh traffic).
			IpPermission ipPermission = new IpPermission().withIpProtocol("tcp").withFromPort(new Integer(22))
					.withToPort(new Integer(450)).withIpRanges(ipRanges);
			...
			Tag nameTag = new Tag("Name", "Instance");
